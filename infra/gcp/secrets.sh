...
_ensure_secret "SUPABASE_ANON_KEY" "replace-with-mobile-env-anon-key"
_ensure_secret "SUPABASE_SERVICE_ROLE_KEY" "replace-with-supabase-dashboard-service-role-key"
...
echo ""
echo "Sync Supabase from card_scan/mobile/.env (required for Expo JWT auth on /scan):"
echo "  export GCP_PROJECT=$GCP_PROJECT"
echo "  ./infra/gcp/sync_supabase_from_mobile_env.sh"
echo ""
echo "Set Supabase service role (required for scan_jobs/scanned_cards mirror):"
echo "  # If you see NOT_FOUND, create the secret first:"
echo "  export GCP_PROJECT=$GCP_PROJECT && ./infra/gcp/secrets.sh"
echo "  # Then add your real key from Supabase Dashboard → Settings → API:"
echo "  echo -n 'YOUR_SERVICE_ROLE_KEY' | gcloud secrets versions add SUPABASE_SERVICE_ROLE_KEY --data-file=-"
echo ""
